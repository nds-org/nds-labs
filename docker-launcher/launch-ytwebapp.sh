...
CELERY_CID=$(docker run -d -v /var/lib/docker:/var/lib/docker --privileged -p 8888 --link $RABBIT_HOST:amq --link
${ICAT_NAME}:${ICAT_NAME} -e BROKER_URL=${BROKER_URL} ${CELERY_IMAGE})
